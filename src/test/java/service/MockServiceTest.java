...
public class MockServiceTest {
    ...
    @Test
    public void always_return_position_mid(){
        Champion champion = mock(Champion.class);
    ...
    public void shouldOneTimeRunForChampionArraySize(){
        ...
        Champion supChamp = mock(Champion.class);
        supChamp.setName("쓰레쉬");
        supChamp.setPosition("서폿");
        supChamp.setHasSkinCount(3);
        ...
        System.out.println("size : " + mockChampions.size());   //가짜 객체이므로 0
    ...
    public void shouldAtLeastTwoTimesForChampionGetName(){
        ...
        System.out.println("Name : " + supChamp.getName());
        System.out.println("Name : " + supChamp.getName());

        verify(supChamp, atLeast(2)).getName();
    }
    ...
    public void shouldAtMostThreeTimesForChampionGetName(){
        ...
        System.out.println("Name : " + supChamp.getName());
        System.out.println("Name : " + supChamp.getName());
        verify(supChamp, atMost(3)).getName();
    }
    // 4-3. champion 객체에서 이름을 저장하는 로직이 실행되지 않았으면 Pass 하는 로직을 작성하세요.
    @Test
    public void shouldNeverSetName(){
        Champion supChamp = mock(Champion.class);
        supChamp.setPosition("서폿");
        supChamp.setHasSkinCount(3);

        verify(supChamp, never()).setName(anyString());
    }
    ...
    public void shouldOneTimeRunForChampionGetName(){
        ...
        verify(supChamp, timeout(200).atLeastOnce()).setName(anyString());
    }
    ...
    public void 갯수(){
        ...
        champions.add(new Champion("쓰레쉬", "서폿", 5));
    ...
    public void 가짜챔피언객체리턴(){
        when(mockService.findByName(anyString())).thenReturn(new Champion("쓰레쉬", "서폿", 3));
    ...
    @Test
    public void 가짜챔피언들호출횟수(){
        List<Champion> champions = new ArrayList<>();
        champions.add(new Champion("쓰레쉬", "서폿", 3));
        champions.add(new Champion("시비르", "바텀", 3));
        champions.add(new Champion("조이", "미드", 5));
        when(mockService.findAllChampions()).thenReturn(champions);
        assertThat(mockService.findAllChampions().get(0).getName(), is("쓰레쉬"));
